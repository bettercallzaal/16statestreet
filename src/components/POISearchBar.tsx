...
import {
  MapPin, BookOpen, Utensils, Store, Laptop, Trees,
...
function inferCategory(poiCategories: string[]): POICategory {
  ...
  if (cats.some(c => ['library', 'book store', 'bookshop'].includes(c))) return 'library';
  if (cats.some(c => ['park', 'garden', 'playground', 'trail', 'nature'].includes(c))) return 'park';
  if (cats.some(c => ['restaurant', 'food', 'cafe', 'coffee', 'bakery', 'diner', 'pizza', 'fast food', 'breakfast', 'bar', 'pub', 'brewery'].includes(c))) return 'food';
  if (cats.some(c => ['shop', 'store', 'retail', 'boutique', 'market', 'gallery'].includes(c))) return 'shop';
  if (cats.some(c => ['coworking', 'office', 'coworking space', 'business center'].includes(c))) return 'work';
  return 'pin';
}
        ...
        <div className="p-3 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative w-9 h-5 rounded-full transition-colors cursor-pointer ${
                isPublic ? 'bg-rose-500' : 'bg-slate-600'
              }`}
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-500 hover:bg-rose-600 active:bg-rose-600 disabled:opacity-50 text-white text-xs font-medium rounded-lg transition-colors cursor-pointer"
            >
              {saving ? (
