...
export function useFriends() {
  ...
  const refreshFriends = useCallback(async () => {
    ...
    friendshipsA?.forEach((f: { user_b: string }) => friendIds.add(f.user_b));
    friendshipsB?.forEach((f: { user_a: string }) => friendIds.add(f.user_a));
    ...
    const profileMap = new Map<string, Friend>(
      (profiles ?? []).map((p: Friend) => [p.user_id, p] as [string, Friend])
    );
