...
import { HAS_SUPABASE, MAX_POIS } from '@/lib/constants';
...
export function usePOIs() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const friendUserIds = [...new Set(poiList.filter((p: POI) => p.user_id !== user.id).map((p: POI) => p.user_id))];
      if (friendUserIds.length > 0) {
        const { data: profiles } = await supabase
          .from('profiles')
          .select('user_id, display_name')
          .in('user_id', friendUserIds);
        setOwnerNames(new Map((profiles ?? []).map((p: { user_id: string; display_name: string | null }) => [p.user_id, p.display_name ?? 'Friend'])));
      } else {
        setOwnerNames(new Map());
      }
  ...
  const addPOI = useCallback(
    async (poi: {
      ...
      if (!HAS_SUPABASE || !user || pois.filter((p) => p.user_id === user.id).length >= MAX_POIS) return null;
