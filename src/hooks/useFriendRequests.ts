...
export function useFriendRequests({ refreshFriends }: UseFriendRequestsOptions) {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    incoming?.forEach((r: { sender_id: string }) => userIds.add(r.sender_id));
    outgoing?.forEach((r: { receiver_id: string }) => userIds.add(r.receiver_id));
    ...
    if (userIds.size > 0) {
      ...
      profileMap = new Map(
        (profiles ?? []).map((p: { user_id: string; display_name: string | null; email: string | null; x_handle: string | null }) => [p.user_id, { display_name: p.display_name, email: p.email, x_handle: p.x_handle }])
      );
    }

    const incomingWithProfiles: FriendRequest[] = (incoming ?? []).map((r: FriendRequest) => ({
      ...r,
      sender_profile: profileMap.get(r.sender_id),
    }));

    const outgoingWithProfiles: FriendRequest[] = (outgoing ?? []).map((r: FriendRequest) => ({
      ...r,
