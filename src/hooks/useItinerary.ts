...
import { STORAGE_KEYS, HAS_SUPABASE } from '@/lib/constants';
...
export function useItinerary() {
  ...
  // In demo mode (no Supabase), use localStorage-only mode
  useEffect(() => {
  ...
  // Sync with Supabase when user logs in (only if Supabase is configured)
  useEffect(() => {
    if (!HAS_SUPABASE || !user || !loaded) return;
  ...
  useEffect(() => {
    ...
    if (HAS_SUPABASE && user && initialSyncDone.current) {
